...
void drawNoseAndMouth(int x, int y) {
  ...
  int filterLen = 24; 
  tft.drawLine(x, y, x, y + filterLen, ILI9341_BLACK);
  int bocaWidth = 60;
  int bocaDepth = 20;
  int baseFilterY = y + filterLen;
  tft.drawLine(x, baseFilterY, x - bocaWidth / 2, baseFilterY + bocaDepth / 2, ILI9341_BLACK);
  tft.drawLine(x - bocaWidth / 2, baseFilterY + bocaDepth / 2, x - bocaWidth, baseFilterY, ILI9341_BLACK);
  tft.drawLine(x, baseFilterY, x + bocaWidth / 2, baseFilterY + bocaDepth / 2, ILI9341_BLACK);
  tft.drawLine(x + bocaWidth / 2, baseFilterY + bocaDepth / 2, x + bocaWidth, baseFilterY, ILI9341_BLACK);
}
